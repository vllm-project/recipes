...
- **Multi-node uses vLLM mp (multiprocessing) backend**, not Ray — matches the vos deployment pattern. For TP/TEP every node runs the same command varying `--node-rank`; rank>0 adds `--headless`. For DEP, Node N adds `--data-parallel-start-rank = N × gpus_per_node`. The UI shows exactly two tabs (Head + Node 1) as a 2-node example.
