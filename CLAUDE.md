...
- `features` — map of `{feature_key: { description, args[], label? }}`. Naming convention: `tool_calling`, `reasoning`, `spec_decoding` (the unified key for MTP / Eagle3 / ERNIE-MTP — **don't use `mtp`**, it was renamed across all recipes). Each feature is a boolean toggle by default. Optional `label` overrides the pill text (default: title-cased key), e.g. `kv_offloading` → `label: "KV offloading (LMCache)"`.
  ...
  - **Per-strategy gating** — a feature may declare `strategies: [<strategy_id>, …]` (allowlist; absent = every strategy). Gate a feature when its args collide with a strategy's own flags or its setup doesn't generalize — e.g. `kv_offloading`'s LMCache `--kv-transfer-config` vs `pd_cluster`'s NixlConnector config (features emit last, so last-wins dedupe would clobber the PD wiring) and its node-local companion server rules out multi-node. On excluded strategies the pill renders disabled + struck-through ("Not available with this strategy"); the toggle state is preserved and synthesis independently skips the feature's args and companion. Helper: `isFeatureAllowedForStrategy` in `command-synthesis.js`.
  - **Companion process** — a feature may declare `companion: { label, description?, command }` for a helper that must run alongside `vllm serve` on the same node (e.g. `kv_offloading`'s `lmcache server`). While the feature is on, the single-node command block renders PD-style tabs (`vLLM Serve · <label>`); the companion tab shows the raw command (no docker wrap, no env prelude) with the description as a `#` comment line. Carried in the JSON API as `companions[]` on single-node renderings. Keep the full command (and any healthcheck step) in the companion — not in the guide — so users copy it from the tab.
