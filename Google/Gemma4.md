...
```
https://<your-workspace>--vllm-gemma4-serve.modal.run
```

You can then query it with any OpenAI-compatible client using that URL as the `base_url`.

### Test (without deploying)

To spin up a temporary replica and run the built-in test against it:

```bash
modal run gemma4_modal.py
```

This runs the `test()` local entrypoint — it starts a fresh server on Modal, waits for it to be healthy, sends two chat messages, and streams the responses back to your terminal.
