...
# (1) bf16 activation for MoE. matching reference precision.
# export VLLM_USE_FLASHINFER_MOE_MXFP4_BF16=1
# (2) mxfp8 activation for MoE. minor accuracy drop with better performance.
...
The two main tunable configs for gpt-oss-120b are the `--tensor-parallel-size` (TP) and `--max-num-seqs` (BS).